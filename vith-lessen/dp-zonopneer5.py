...
def getzonneklok() :
    ...
    uurr = 10
    minuutt = 53
    dawntijd = dawntijd.replace(hour=uurr, minute=minuutt  , second= 0)
    sunrisetijd = dawntijd.replace(hour=uurr, minute=minuutt+1, second= 40 )
    sunsettijd =dawntijd.replace(hour=uurr, minute=minuutt+2 , second= 20)
    dusktijd = dawntijd.replace(hour=uurr, minute=minuutt+3 , second= 0)
    '''
...
try:
    ...
    while True:
        ...
        komendemiddernacht = dawntijdmorgen.replace(hour=0 , minute=0 , second= 0)
        stopupdtijd = dawntijdmorgen.replace(hour=0, minute=1, second=10)
        if (rtc > komendemiddernacht) and (rtc < stopupdtijd):
        ...
        if (rtc > sunrisetijd) and (rtc < sunsettijd):
        ...
        if (rtc > sunsettijd) and (rtc < dusktijd):
        ...
        if (rtc > dusktijd) and (rtc < komendemiddernacht):
            watishetnubuiten = "nacht en naar middernacht ant gaan"
            #print(watishetnubuiten)
            #getzonneklok()
            try:
        ...
        if   (rtc < dawntijd):
            watishetnubuiten = "nacht na middernacht en voor vroege ochtend"
            #print(watishetnubuiten)
            try:
                GPIO.output(dag0nacht1, 1)
                GPIO.output(schemer, 0)
